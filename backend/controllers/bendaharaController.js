...
exports.getDashboard = async (req, res) => {
    try {
        // Total Kas Anggota
        const [kasResult] = await pool.query('SELECT SUM(total_terbayar) as total_kas FROM keuangan_kas_anggota');
        const totalKasAnggota = kasResult[0].total_kas || 0;

        // Total Saldo Semua Kegiatan
        const [kegiatanResult] = await pool.query('SELECT SUM(saldo_awal) as total_saldo_awal FROM keuangan_kegiatan');
...
exports.getKasAnggota = async (req, res) => {
    try {
        const query = `
            SELECT 
                u.id as user_id, u.nama_lengkap, u.nim, u.jabatan,
                COALESCE(k.target_nominal, 0) as target_nominal,
                COALESCE(k.total_terbayar, 0) as total_terbayar,
                k.bulan_dibayar
            FROM users u
            LEFT JOIN keuangan_kas_anggota k ON u.id = k.user_id
            WHERE u.role != 'admin' OR u.role = 'admin' -- Get everyone basically, or filter if needed
...
exports.updateKasAnggota = async (req, res) => {
    try {
        ...
        const [existing] = await pool.query('SELECT id FROM keuangan_kas_anggota WHERE user_id = ?', [user_id]);
        
        if (existing.length > 0) {
            await pool.query(
                'UPDATE keuangan_kas_anggota SET target_nominal = ?, total_terbayar = ? WHERE user_id = ?',
                [target_nominal, total_terbayar, user_id]
            );
        } else {
            await pool.query(
                'INSERT INTO keuangan_kas_anggota (user_id, target_nominal, total_terbayar) VALUES (?, ?, ?)',
                [user_id, target_nominal, total_terbayar]
            );
        }
...
exports.toggleKasBulan = async (req, res) => {
    try {
        ...
        const [existing] = await pool.query('SELECT id, total_terbayar, bulan_dibayar FROM keuangan_kas_anggota WHERE user_id = ?', [user_id]);
        ...
        if (existing.length === 0) {
            ...
            await pool.query(
                'INSERT INTO keuangan_kas_anggota (user_id, target_nominal, total_terbayar, bulan_dibayar) VALUES (?, 0, ?, ?)',
                [user_id, initialTerbayar, initialBulan]
            );
...
exports.resetKasBulan = async (req, res) => {
    try {
        ...
        await pool.query(
            'UPDATE keuangan_kas_anggota SET total_terbayar = 0, bulan_dibayar = ? WHERE user_id = ?',
            ['[]', id]
        );
...
exports.getKegiatan = async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT * FROM keuangan_kegiatan ORDER BY created_at DESC');
        res.json(rows);
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Server Error' });
...
exports.createKegiatan = async (req, res) => {
    try {
        ...
        await pool.query(
            'INSERT INTO keuangan_kegiatan (nama_kegiatan, deskripsi, saldo_awal, created_by) VALUES (?, ?, ?, ?)',
            [nama_kegiatan, deskripsi, saldo_awal || 0, created_by]
        );
...
exports.createTransaksi = async (req, res) => {
    try {
        ...
        await pool.query(
            'INSERT INTO keuangan_transaksi (kegiatan_id, tanggal, jenis, nominal, keterangan, bukti_url, created_by) VALUES (?, ?, ?, ?, ?, ?, ?)',
            [id, tanggal, jenis, nominal, keterangan, bukti_url, created_by]
        );
